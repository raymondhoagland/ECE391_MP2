...
unsigned char * text_to_image(unsigned char * input){
    unsigned char *buf;
    buf = malloc(40*16*sizeof(unsigned char *));
    int i, k, indx = 0;
    for(i = 0; i < 40*16; i+=16){
        unsigned char *table_lookup = font_data[input[indx]];
        for(k = 0; k < 16; k++){
            buf[i+k] = table_lookup[k];
        }
        indx++;
    }
    return buf;
...
int main(){
    unsigned char *buf = text_to_image("hellohellohellohellohellohellohellohello");
    int i, count = 0;
    for(i = 0; i < 40*16; i++){
        printf("%i ", buf[i]);
        count++;
